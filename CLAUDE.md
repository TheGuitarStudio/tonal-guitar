...
`tuning.ts`, `shape.ts`, `pattern.ts`, `notation.ts`, `walker.ts`, `sequence.ts`, `arpeggio.ts`, `connect.ts`, `chord-scale.ts`, `changeset.ts`, `data/*` — **except** `data/caged-scales-minor.ts` and `data/pentatonic-minor.ts`, which call `relabelShape` at import time and therefore transitively require `@tonaljs/interval` via `transform.ts` (see below). Every other `data/*` file remains zero-Tonal-dep. `chord-scale.ts` has no imports at all; `changeset.ts` imports only types from `./shape`.
