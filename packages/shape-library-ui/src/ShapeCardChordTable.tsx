/**
 * Ported from `site/app/shapes/components/ShapeCardChordTable.tsx`.
 * Chord shape's interval/finger/fret table plus barre summary. Purely
 * presentational: all derived values (string count, mismatch highlighting,
 * source-fret visibility) are recomputed here from the raw props. Read-only:
 * never emits `data-tg-edit`.
 */
import { absoluteBarreFret, CHECK_GEOMETRY_MISMATCH, gripBaseFret } from "tonal-guitar";
import type { ChordShape, ShapeAuditIssue } from "tonal-guitar";

export interface ShapeCardChordTableProps {
  chordShape: ChordShape;
  builtFrets: (number | null)[];
  sourceFrets: (number | null)[] | undefined;
  gripRoot: string | undefined;
  renderRoot: string;
  issues: ShapeAuditIssue[];
}

function mismatchedStringsFor(issues: ShapeAuditIssue[]): Set<number> {
  const issue = issues.find((i) => i.id === CHECK_GEOMETRY_MISMATCH);
  const raw = issue?.details?.mismatchedStrings;
  const validated = Array.isArray(raw) ? raw.filter((v): v is number => typeof v === "number") : [];
  return new Set(validated);
}

function intervalCell(shape: ChordShape, i: number): string {
  const ivl = shape.strings[i];
  return ivl === null ? "x" : ivl;
}

function fingerCell(shape: ChordShape, i: number): string {
  if (shape.strings[i] === null) return "x";
  const finger = shape.fingers[i];
  if (finger === 0) return "0";
  if (finger === null || finger === undefined) return "";
  return String(finger);
}

function fretCell(shape: ChordShape, frets: (number | null)[], i: number): string {
  if (shape.strings[i] === null) return "x";
  const fret = frets[i];
  return fret === null || fret === undefined ? "" : String(fret);
}

/**
 * D-010: `barre.fret` is an offset from the grip base, not an absolute fret
 * — render both the offset (the stored value) and the absolute fret it
 * resolves to at `root` (`absoluteBarreFret(barre, gripBase)`).
 */
function barreLabel(barre: ChordShape["barres"][number], gripBase: number, root: string): string {
  const absoluteFret = absoluteBarreFret(barre, gripBase);
  return `finger ${barre.finger}: strings ${barre.fromString}–${barre.toString} @ offset ${barre.fret} (fret ${absoluteFret} at ${root})`;
}

export function ShapeCardChordTable({
  chordShape,
  builtFrets,
  sourceFrets,
  gripRoot,
  renderRoot,
  issues,
}: ShapeCardChordTableProps) {
  const stringIndexes = Array.from({ length: chordShape.strings.length }, (_, i) => i);
  const showSourceFrets = sourceFrets !== undefined;
  const sourceAtDifferentRoot = gripRoot !== undefined && gripRoot !== renderRoot;
  const mismatchedStrings = mismatchedStringsFor(issues);
  const builtGripBase = gripBaseFret(builtFrets);

  return (
    <div className="tg-table">
      <table>
        <thead>
          <tr>
            <th scope="col">string</th>
            {stringIndexes.map((i) => (
              <th key={i} scope="col">
                {i}
              </th>
            ))}
          </tr>
        </thead>
        <tbody>
          <tr>
            <th scope="row">interval</th>
            {stringIndexes.map((i) => (
              <td key={i}>{intervalCell(chordShape, i)}</td>
            ))}
          </tr>
          <tr>
            <th scope="row">finger</th>
            {stringIndexes.map((i) => (
              <td key={i}>{fingerCell(chordShape, i)}</td>
            ))}
          </tr>
          <tr>
            <th scope="row">built fret</th>
            {stringIndexes.map((i) => (
              <td key={i}>{fretCell(chordShape, builtFrets, i)}</td>
            ))}
          </tr>
          {showSourceFrets && (
            <tr>
              <th scope="row">source fret{sourceAtDifferentRoot ? ` (source at ${gripRoot})` : ""}</th>
              {stringIndexes.map((i) => (
                <td key={i} className={mismatchedStrings.has(i) ? "tg-mismatch" : undefined}>
                  {fretCell(chordShape, sourceFrets ?? [], i)}
                  {/* Non-color mismatch cue: the highlight alone is
                      invisible to colorblind and screen-reader users. */}
                  {mismatchedStrings.has(i) && (
                    <>
                      <span aria-hidden="true">*</span>
                      <span className="tg-sr-only"> (mismatch)</span>
                    </>
                  )}
                </td>
              ))}
            </tr>
          )}
        </tbody>
      </table>

      {chordShape.barres.length > 0 && (
        <ul className="tg-barres">
          {chordShape.barres.map((barre, i) => (
            <li key={i}>{barreLabel(barre, builtGripBase, renderRoot)}</li>
          ))}
        </ul>
      )}
    </div>
  );
}
