...
describe("ShapeCard", () => {
  it("renders under renderToString with no window access", () => {
    expect(() => renderToString(<ShapeCard entry={chordEntry} onSelectEntry={() => {}} isSelected={false} />)).not.toThrow();
  });

  it("emits zero data-tg-edit elements without a provider", () => {
    const html = renderToString(<ShapeCard entry={chordEntry} onSelectEntry={() => {}} isSelected={false} />);
  ...
  it("emits zero data-tg-edit elements when capabilities.edit is undefined", () => {
    const html = renderToString(
      <ShapeLibraryProvider capabilities={{}}>
        <ShapeCard entry={chordEntry} onSelectEntry={() => {}} isSelected={false} />
  ...
  it("shows the card as selected via aria-pressed/aria-current", () => {
    const html = renderToString(<ShapeCard entry={chordEntry} onSelectEntry={() => {}} isSelected={true} />);
  ...
  it("renders under renderToString with lazy set, with no window access", () => {
    expect(() =>
      renderToString(<ShapeCard entry={chordEntry} onSelectEntry={() => {}} isSelected={false} lazy />),
    ).not.toThrow();
  });
  ...
  it("renders the real card content (not a placeholder) on the server when lazy without eager", () => {
    ...
    const html = renderToString(<ShapeCard entry={chordEntry} onSelectEntry={() => {}} isSelected={false} lazy />);
    expect(html).not.toContain(chordEntry.name);
  });

  it("renders the real card content on the server when lazy and eager", () => {
    const html = renderToString(
      <ShapeCard entry={chordEntry} onSelectEntry={() => {}} isSelected={false} lazy eager />,
    );
